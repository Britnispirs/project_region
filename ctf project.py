...
class WebScanner:
    ...
    def get_forms(self, url):
        """Извлекает все формы с указанной страницы"""
        response = self.session.get(url)
        soup = BeautifulSoup(response.content, "html.parser")
        return soup.find_all("form")

    def submit_form(self, form, value, url):
        """Отправляет форму с тестовым пейлоадом"""
    ...
    def scan_sql_injection(self, url):
        """Проверка на SQL-инъекцию (Error-based)"""
        sql_payload = "'"
        print(f"[*] Проверка параметров URL на SQLi: {url}")
        
        # Проверка через параметры URL
